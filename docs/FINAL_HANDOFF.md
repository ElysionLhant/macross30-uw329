...
- **人脸缩半（通讯场景监视器头像）**——五轮二分定位：画家 = `0x5e5ea4`，但它同时给 LAYO 对话框（需居中）和帧缓冲空间头像（原公式即正确）打工，**混画**。**已修（门控种子，10 词）**：角点 `(px/A)·fS−fS` 在 fS=0.5 是居中、fS=1.0 是原式，判别 = quad 右缘 x1>1280（整数位掩码，正值浮点位形可比）；跳板 `0x7009d0` 尾分支改道段尾加载器填充（`0xa6ad4c` 7 词洞）算 fS 经 r12 传入，写出器 nop（`0x5e5f08`）落 `writer_frame+0x88`，种子槽（`0x5e5fcc`）改读之。全程只一个 `bl 0x5bba04`（仅碰 r3）保 r12 安全。坑：fsel 需 3 条指令放不进函数内（穷举过）；PPC A-form frB/frC 字段序、lwz opcode=32 都踩过。**剩最后一项**：用户回来看一眼通讯场景（脸+底座同屏）签收。
