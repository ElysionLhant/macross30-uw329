...
### 混画门控终局（2026-08-17：头像问题修复定案 v4，含表情/嘴型动画）
- **取证革命：环形日志洞（路线 B）**。GDB 断点要静态解释器（0.08fps）且存证状态（savestate）路线未验证；RSX 抓包顶点暂存不进快照（附录 C 早有警告）。最终方案：跳板 0x7009d0 改道 61 词日志洞 @0x9e2f8c（596B 零岛），每次写出器调用记 {LR, r3/r5/r6/r7, 8 坐标, 8 UV, 对象头} 进 0xa6c000 的 64 槽环（0xBEEF5EA5 魔数自校验），pymem 全速远程读——**用户正常玩，数据自己流进来**。工具：`tools/uw_ring_read2.py`（单次带纹理名）/ `uw_bg_watch.py`（长监听）/ `uw_talk_watch.py`。
- **描述体彩蛋**：r5 = 元素描述体，+0x1c 是 SSO 字符串（cap@+0x30==0xF 时内联，否则指针）——能直接读出贴图路径（`data/cockpit/hud/...`、`po_ais_w.dds`）。**池化复用警告**：跨场景解读旧 r5 拿到的是别人的名字，必须场景活着时读。
- **判别器定论**（全部数据驱动）：LR 分布 = 除头像外一切走 `0x79674`（含全部九宫格底座！文档旧记"0x4cxxx=九宫格对话框类"系静态误判）；头像底图（`po_*.dds`/`/pk2_sw2_n.dds`）走 `0x4c214`，表情/嘴型叠加格（`po_*_0..3.dds` 小 quad 叠嘴/眼部）走 `0x4c9f0`，`0x4ca64` 未触发。UV 跨度判别器证伪（标题 title_cloud/logo 全 UV；HUD 里 `face_l_mask.dds` 全 UV）；宽度窗证伪（场景间宽度不一）。
- **头像低频重烘焙**：内容不变不重画（顶点缓冲复用），静态画面抓不到——商店头像 30 秒零触发，说话/换装时密集重烘焙。这解释了早期"采集落空"。
